...
namespace Compass
{
	...
	{
	private:
		std::shared_ptr<Tensor<float>> m_Weights;
		std::shared_ptr<Tensor<float>> m_InputVector;
		std::shared_ptr<Tensor<Gradient>> m_GradientVector;
	...
	public:
		FullyConnectedLayer(TensorSize inputSize, uint32_t outputSize)
			...
			m_Weights(std::make_shared<Tensor<float>>(inputSize.Width * inputSize.Height * inputSize.Depth, outputSize, 1))
		{
			m_InputVector = std::make_shared<Tensor<float>>(outputSize, 1, 1);
			m_GradientVector = std::make_shared<Tensor<Gradient>>(outputSize, 1, 1);
		...
		virtual void ComputeGradient(std::shared_ptr<Tensor<float>> nextLayer)
		{
			m_Gradient->Fill(0);
